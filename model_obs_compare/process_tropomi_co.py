...
from utils import (
    standard_value, 
    get_pressure_tropomi, 
    convert_bytes_to_datetimes, 
    get_valid_tropomi, 
    regrid_sw_vertically, 
    cal_avg, 
    save_output_grid_variables_monthly, 
)
...
def read_tropomi_l2(tropomi1,tropomi2):
    ...
    total_col = standard_value(tropomi1,'carbonmonoxide_total_column')[0,:,:]*factor                                  #2d  unit: molec cm-2
    total_col_uncertainty = standard_value(tropomi1,'carbonmonoxide_total_column_precision')[0,:,:]*factor            #2d  unit: molec cm-2
    h2o_col = standard_value(tropomi2['DETAILED_RESULTS'],'water_total_column')[0,:,:]*factor                                             #2d  unit: molec cm-2
    ...
    ak = standard_value(tropomi2['DETAILED_RESULTS'],'column_averaging_kernel')[0,:,:]    #3d  unitless [scanline(changable),ground_pixel(450),layer(34)]
    apriori = standard_value(tropomi2['INPUT_DATA'],'carbonmonoxide_profile_apriori')[0,:,:,:]*factor   #3d  unit: molec cm-2 [scanline(changable),ground_pixel(450),layer(34)]
    ...
    utc_time = np.repeat(utc_time,total_col.shape[1],0).T
    ...
    surf_pres = standard_value(tropomi2['INPUT_DATA'],'surface_pressure')[0,:,:]      #2d  unit: Pa [scanline(changable),ground_pixel(450)]
    ...
    pressure = standard_value(tropomi2['DETAILED_RESULTS'], 'pressure_levels')[0, :] / 100 # 3d hPa [1, scanlines, pixels, levels]
    # pressure = (pressure_levels[:, :, :-1] + pressure_levels[:, :, 1:])/2                         #3d  unit: hPa [scanline(changable),ground_pixel(450),layer(34)]
    # tropopause_index = standard_value(tropomi1,'tm5_tropopause_layer_index')[0,:,:]        #2d  unit: 1 [scanline(changable),ground_pixel(450)]
    ...
    g = 9.81 # m s-2
    MW_dryair = 28.965e-3 # kg mol-1
    NA = 6.02214076e23
    m_dryair = MW_dryair / NA
    N_dryair = (surf_pres / (m_dryair * g))*1e-4 # column dry air number density [molec cm-2]
    XCO = total_col / N_dryair  * 1e9 # ppbv
    ...
    error = {
        'total_col_unc': total_col_uncertainty,
    }
    flag = {
        'sza': sza,
        'vza': vza,
...
def create_output_grid_variables_monthly(gridtimes, gridlats, gridlons):
    ...
    variables = {
        ...
        'XCO_tropomi': np.zeros(shape_3d),            # mixing ratio
...
def process_tropomi_file(file0):
    ...
    for g in range(len(unique_grids)):
        ...
        for p in pixel_idx:
            ...
            partial_output['XCO_tropomi'][day_idx,lat_idx,lon_idx] += pxMixingRatio
            ...
            # we need to drop the top layer of AK variable?
            # if len(pxAK) == len(pxPressure) + 1:
            #     pxAK = pxAK[:-1]
            #     pxApriori = pxApriori[:-1]
            pxAK_GC = regrid_sw_vertically(pxAK,pxPressure,gridPressure)
            
            # applying ak to GEOS-Chem results
            gridVCD_ak = sum(gridVCDL*pxAK_GC) # GC total column with AK applied (mol m-2)
...
output_path = '/n/holylfs05/LABS/jacob_lab/Users/mhe/Obs_data/TROPOMI_regrid/CO/'
...
for yyyymm in tropomi_months:
    ...
    # results = process_tropomi_file(file_list[0])
